...
{
	const TX_POWER_TUNING_ENTRY_STRUCT  * const TxPowerTuningTable = pAd->chipCap.TxPowerTuningTable_2G;
	...
	IN CHAR				Rssi)
{
			...
			(pAd->StaCfg.AntS == 1))		
		{
			...
			DBGPRINT(RT_DEBUG_TRACE,("AntS: AvgTxErrorRatio = %ld , TxErrorRatio = %ld, AccuTxTotalCnt = %ld, Rssi = %d\n",AvgTxErrorRatio, TxErrorRatio, AccuTxTotalCnt, (int)Rssi));
			...
			{
				DBGPRINT(RT_DEBUG_TRACE, ("AntS: Train State: AntennaDiversityCount = %ld\n", pAd->AntennaDiversityInfo.AntennaDiversityCount));
				...
				else if (pAd->AntennaDiversityInfo.AntennaDiversityCount == 1)
				{
					...
					DBGPRINT(RT_DEBUG_TRACE,("AntS: AntSAuxDelta = %ld, AntSRssiFactor = %ld, AntSPERFactor = %ld\n",
					...
					if (Rssi >= -75)
					{
						if (Rssi < -35)
						{
							if (pAd->AntennaDiversityInfo.Rssi[0] > pAd->AntennaDiversityInfo.Rssi[1])
							{
								...
								DBGPRINT(RT_DEBUG_TRACE,("AntS: Good Rssi is at Main !! ++%ld\n",(diffRssi*Ant0TotalPacket/pAd->StaCfg.AntSRssiFactor))); 
							}
							else
							{
								...
								DBGPRINT(RT_DEBUG_TRACE,("AntS: Good Rssi is at Aux !!++%ld\n",(diffRssi*Ant1TotalPacket/pAd->StaCfg.AntSRssiFactor))); 
							}
						}
						...
						if (pAd->AntennaDiversityInfo.AntennaDiversityPER[0] < pAd->AntennaDiversityInfo.AntennaDiversityPER[1])
						{
							...
							DBGPRINT(RT_DEBUG_TRACE,("AntS: Good PER is at Main !! ++%ld\n", (diffPER*Ant0TotalPacket/pAd->StaCfg.AntSPERFactor))); 
						}
						else
						{
							...
							DBGPRINT(RT_DEBUG_TRACE,("AntS: Good PER is at Aux !! ++%ld\n", (diffPER*Ant1TotalPacket/pAd->StaCfg.AntSPERFactor))); 
						}						
						DBGPRINT(RT_DEBUG_TRACE,("AntS: Delta Packet = %ld,%ld, diffRssi = %d, diffPER = %d\n",(diffRssi*Ant0TotalPacket/pAd->StaCfg.AntSRssiFactor),(diffPER*Ant1TotalPacket/pAd->StaCfg.AntSPERFactor),(int)diffRssi, (int)diffPER)); 
					}
					...
					DBGPRINT(RT_DEBUG_TRACE,("AntS: Stop Train State: PER[Main] = %ld, PER[Aux] = %ld\n",pAd->AntennaDiversityInfo.AntennaDiversityPER[0],pAd->AntennaDiversityInfo.AntennaDiversityPER[1])); 
					DBGPRINT(RT_DEBUG_TRACE,("AntS: TotalTxCount[Main] = %ld, TotalTxCount[Aux] = %ld\n",pAd->AntennaDiversityInfo.AntennaDiversityTxPacketCount[0],pAd->AntennaDiversityInfo.AntennaDiversityTxPacketCount[1])); 
					DBGPRINT(RT_DEBUG_TRACE,("AntS: TotalRxCount[Main] = %ld, TotalRxCount[Aux] = %ld\n",pAd->AntennaDiversityInfo.AntennaDiversityRxPacketCount[0],pAd->AntennaDiversityInfo.AntennaDiversityRxPacketCount[1])); 
