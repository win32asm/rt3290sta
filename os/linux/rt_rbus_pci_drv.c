    ...
    IN	VOID				*pAdSrc)
{
	...
	if (device_id == NIC3290_PCIe_DEVICE_ID) {
		...
		RTMP_IO_READ32(pAd, reg, &MacCsr0);
		pAd->MACVersion = MacCsr0;
		DBGPRINT(RT_DEBUG_TRACE, ("pAd->MACVersion  = 0x%X\n",pAd->MACVersion));
...
#ifdef RT3290
		(device_id ==  NIC3290_PCIe_DEVICE_ID)||
#endif /* RT3390 */
		 0)
	{
...
#endif /* RT3290 */
		RTMP_IO_READ32(pAd, reg, &MacCsr0);
		DBGPRINT(RT_DEBUG_TRACE, ("MacCsr0 = 0x%X\n",MacCsr0));			
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	if (IS_RT3290(pAd))
	{
		...
		AsicSendCommandToMcu(pAd, TRUE, 0x83, 0xff, 0, 0);
	}
	else
#endif
...
*/
	...
	DBGPRINT(RT_DEBUG_ERROR, ("RTMPrt3xSetPCIePowerLinkCtrl.===> %x\n", (UINT)pAd->StaCfg.PSControl.word));
